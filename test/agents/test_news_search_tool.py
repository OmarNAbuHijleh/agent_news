...
from src.agents.agent_tools.news_search_tool import search_news
...
def test_search_news_returns_error_when_api_key_missing(monkeypatch):
...
def test_search_news_passes_query_and_api_key_as_params(monkeypatch):
...
def test_search_news_returns_headline_date_url_and_stripped_body(monkeypatch):
    monkeypatch.setattr(news_search_tool_module, "GUARDIAN_API_KEY", "test-key")
    guardian_results = [
        {
...
def test_search_news_truncates_body_to_the_configured_max_length(monkeypatch):
    monkeypatch.setattr(news_search_tool_module, "GUARDIAN_API_KEY", "test-key")
...
def test_search_news_returns_error_dict_on_http_failure(monkeypatch):
    monkeypatch.setattr(news_search_tool_module, "GUARDIAN_API_KEY", "test-key")
